...
void bang_load(raft::device_resources handle, char* indexfile_path_prefix)
{
	...
	float *pqTable = NULL;
	pqTable = (float*) malloc(sizeof(float) * (CODEBOOK_SIZE * objIndexLoad.D)); // Contains pivot coordinates
	if (NULL == pqTable)
	{
	...
	in1.seekg(8);
	in1.read((char*)pqTable,sizeof(float)*CODEBOOK_SIZE*objIndexLoad.D);
	in1.close();
...
template void bang_load<uint8_t>(raft::device_resources, char* );
// ToDo: Add definitions for other dataset types e.g. int8_t for MSSPACEV dataset after testing

void bang_load_c(raft::device_resources handle, char* pszPath)
{
	bang_load<uint8_t>(handle, pszPath);
}
...
void bang_query_c(raft::device_resources handle, uint8_t* query_array, int num_queries,
					result_ann_t* nearestNeighbours,
					float* nearestNeighbours_dist )
{
