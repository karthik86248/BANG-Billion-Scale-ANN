...
#define MAX_R 64 // Degree bound of the graph. compile-time definition to enable usage in arrays. Update it to match m_objInputData.R
				 // that is read dynamically. The current code needs to be re-compiled if the degree bound is changed from 64.  
